...
class _VolumeConverterScreenState extends State<VolumeConverterScreen> {
  ...
  // Selected volumes
  String vol1 = "Cubic metre m\u00B3";
  String vol2 = "Cubic centimetre cm\u00B3";
  int activeRowIndex = 0;

  // Conversion rates (Relative to 1.0 Cubic Metre)
  final Map<String, double> volumeRates = {
    "Cubic metre m\u00B3": 1.0,
    "Cubic centimetre cm\u00B3": 1000000.0,
    "Litre L": 1000.0,
    "Millilitre mL": 1000000.0,
    "Gallon (US) gal": 264.172052,
    "Quart (US) qt": 1056.68821,
    "Pint (US) pt": 2113.37642,
  };
            ...
            Expanded(
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  onChanged: onChanged,
                  items: volumeRates.keys.map<DropdownMenuItem<String>>((
                    String value,
                  ) {
  ...
  Widget build(BuildContext context) {
    ...
    double activeRate = volumeRates[activeRowIndex == 0 ? vol1 : vol2] ?? 1.0;

    double currentVal = double.tryParse(display) ?? 0;

    // Normalize to cubic metres
    double normalizedValInM3 = currentVal / activeRate;

    // Convert back up for both rows
    double val1 = normalizedValInM3 * (volumeRates[vol1] ?? 1.0);
    double val2 = normalizedValInM3 * (volumeRates[vol2] ?? 1.0);
                ...
                child: Row(
                  children: [
                    GestureDetector(
                      ...
                      child: Icon(Icons.arrow_back, color: currentTheme.textColor),
                    ),
                    Expanded(
                      child: Text(
                        'Volume',
