...
class _TimeConverterScreenState extends State<TimeConverterScreen> {
  ...
  // Selected times
  String time1 = "Minute min";
  String time2 = "Second s";
  int activeRowIndex = 0;

  // Conversion rates (In Seconds)
  final Map<String, double> timeRates = {
          ...
          children: [
            DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                onChanged: onChanged,
                items: timeRates.keys.map<DropdownMenuItem<String>>((
                  String value,
                ) {
  ...
  Widget build(BuildContext context) {
    ...
    double activeRate = timeRates[activeRowIndex == 0 ? time1 : time2] ?? 1.0;

    double currentVal = double.tryParse(display) ?? 0;

    // Normalize to seconds
    // Since rate is "how many seconds in this unit",
    // val in seconds = val in unit * rate.
    double normalizedValInSec = currentVal * activeRate;

    // Convert back up for both rows
    // val in target unit = val in seconds / target unit rate
    double val1 = normalizedValInSec / (timeRates[time1] ?? 1.0);
    double val2 = normalizedValInSec / (timeRates[time2] ?? 1.0);
                ...
                child: Column(
                  children: [
                    _buildTimeRow(
                      ...
                      val1Str,
                      (val) {
                        if (val != null) setState(() => time1 = val);
                      },
                    ...
                    _buildTimeRow(
                      ...
                      val2Str,
                      (val) {
                        if (val != null) setState(() => time2 = val);
                      },
