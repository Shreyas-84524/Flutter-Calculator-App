...
class _SpeedConverterScreenState extends State<SpeedConverterScreen> {
  ...
  // Selected speeds
  String speed1 = "Metre per second m/s";
  String speed2 = "Kilometre per second km/s";
  int activeRowIndex = 0;

  // Conversion rates (Relative to 1.0 m/s)
  final Map<String, double> speedRates = {
    "Metre per second m/s": 1.0,
    "Kilometre per second km/s": 0.001,
    "Kilometre per hour km/h": 3.6,
    "Mile per hour mph": 2.236936,
    "Foot per second ft/s": 3.280840,
    "Knot": 1.943844,
  };
            ...
            Expanded(
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  onChanged: onChanged,
                  items: speedRates.keys.map<DropdownMenuItem<String>>((
                    String value,
                  ) {
  ...
  Widget build(BuildContext context) {
    ...
    double activeRate =
        speedRates[activeRowIndex == 0 ? speed1 : speed2] ?? 1.0;

    double currentVal = double.tryParse(display) ?? 0;

    // Normalize to m/s
    double normalizedValInMs = currentVal / activeRate;

    // Convert back up for both rows
    double val1 = normalizedValInMs * (speedRates[speed1] ?? 1.0);
    double val2 = normalizedValInMs * (speedRates[speed2] ?? 1.0);
                ...
                child: Column(
                  children: [
                    _buildSpeedRow(
                      ...
                      val1Str,
                      (val) {
                        if (val != null) setState(() => speed1 = val);
                      },
                    ...
                    _buildSpeedRow(
                      ...
                      val2Str,
                      (val) {
                        if (val != null) setState(() => speed2 = val);
                      },
