...
class _AreaConverterScreenState extends State<AreaConverterScreen> {
  ...
  // Selected areas
  String area1 = "Square metre m\u00B2";
  String area2 = "Square centimetre cm\u00B2";
  int activeRowIndex = 0;

  // Conversion rates (Relative to 1.0 Square Metre)
  final Map<String, double> areaRates = {
    "Square metre m\u00B2": 1.0,
    "Square centimetre cm\u00B2": 10000.0,
    "Square kilometre km\u00B2": 0.000001,
    "Square inch in\u00B2": 1550.0031,
    "Square foot ft\u00B2": 10.7639104,
    "Square yard yd\u00B2": 1.19599005,
    "Acre ac": 0.000247105381,
    "Hectare ha": 0.0001,
  };
          ...
          children: [
            DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                onChanged: onChanged,
                items: areaRates.keys.map<DropdownMenuItem<String>>((
                  String value,
                ) {
  ...
  Widget build(BuildContext context) {
    ...
    double activeRate = areaRates[activeRowIndex == 0 ? area1 : area2] ?? 1.0;

    double currentVal = double.tryParse(display) ?? 0;

    // Normalize to square meters
    double normalizedValInSqM = currentVal / activeRate;

    // Convert back up for both rows
    double val1 = normalizedValInSqM * (areaRates[area1] ?? 1.0);
    double val2 = normalizedValInSqM * (areaRates[area2] ?? 1.0);
                ...
                child: Column(
                  children: [
                    _buildAreaRow(
                      ...
                      val1Str,
                      (val) {
                        if (val != null) setState(() => area1 = val);
                      },
                    ...
                    _buildAreaRow(
                      ...
                      val2Str,
                      (val) {
                        if (val != null) setState(() => area2 = val);
                      },
